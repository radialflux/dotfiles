...
parse_git_state() {
  ...
  if [[ UNTRACKED_COUNT -gt 0 ]]; then
    ...
    GIT_STATE="$GIT_STATE$GIT_PROMPT_UNTRACKED$UNTRACKED_COUNT"
    SHOULD_INSERT_SPACE=1
  fi
...
git_prompt_string() {
  ...
  [ -n "$git_where" ] && echo " $GIT_PROMPT_PREFIX${git_where#(refs/heads/|tags/)}$(parse_git_ahead)$(parse_git_state)$GIT_PROMPT_SUFFIX"
}

PROMPT='${_prompt_sorin_pwd}$(git_prompt_string)${editor_info[keymap]} '
